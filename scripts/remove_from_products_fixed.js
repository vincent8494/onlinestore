...
async function removeProducts() {
  try {
    ...
    // Pattern to match the existingProducts array
    const productsArrayPattern = /(const\s+existingProducts\s*=\s*\[)([\s\S]*?)(\];)/;
    const match = content.match(productsArrayPattern);
    
    if (!match) {
      console.log('Could not find existingProducts array in Products.tsx');
      return;
    }
    ...
    const productObjects = productsText
      .split(/\{\s*\n/)
      .map(s => s.trim())
      .filter(s => s && !s.startsWith('//') && s.includes('name:'))
      .map(s => `{\n${s}`.trim())
      .filter(s => s.length > 2);
    
    console.log(`Found ${productObjects.length} products in the array`);
    ...
    const filteredProducts = productObjects.filter(product => {
      ...
      const shouldKeep = !PRODUCTS_TO_REMOVE.includes(productName);
      if (!shouldKeep) {
